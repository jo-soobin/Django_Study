...
def reviewCrawling() :
    ...
    chrome_path = 'C:\\projects\\semigit\\Django_Study\\semivenv\\105\\chromedriver.exe'
    driver = webdriver.Chrome(chrome_path)

    mvdf = pd.DataFrame(columns=['mvcode', 'index', 'imgpath'])
    df = pd.DataFrame(columns=['mvcode', 'index', 'review', 'star', 'emoji', 'img', 'create_date'])
    today = timezone.now()
    ...
    try:
        ...
        for code, title in zip(img_codes, img_titles):
            ...
            for t, c in zip(img_type, img_cnt):
                ...
                for click_cnt in range(c):
                    ...
                    mvdf.append({'mvcode':movie_cnt, 'index':click_cnt, 'imgpath':img.get_attribute('src')})
        ...
        for code, title in zip(codes, titles):
            ...
            for i in range(1,6): # 1~6 페이지 리뷰 수집
                ...
                for idx, i in enumerate(review):
                    ...
                    df.append({'mvcode':code, 'index':idx, 'review':title, 'star':point, 'emoji':emoji, 'img':mvdf[mvdf['mvcode']==count].sample(n=1), 'create_date':today})
        ...
        for i in range(len(df)) :
            data = df.iloc[i]
            r = reviewModel(mvcode=data['mvcode'], index=data['index'], review=data['review'], star=data['star'], emoji=data['emoji'], img=data['img'], create_date=today)
            r.save()
